...
const onContextMenu = (e: MouseEvent,callback:Function) => {
                ...
                onClick: () => {
                    Modal.confirm({
                        ...
                        onOk: () => {
                            ...
                            // 检查节点是否存在
                            let arrSources = global.getAllConnections().map(item => item.sourceId)
                            let arrTargets = global.getAllConnections().map(item => item.targetId)
                            let newArr = [...arrSources, ...arrTargets]
                            let arr = Array.from(new Set(newArr))
                            if (arr.includes(nodeId)) {
                                Message.error("该节点已被连线，无法删除！")
                                console.error(`Node ${nodeId} is still connected to other nodes.`);
                                return;
                            }
